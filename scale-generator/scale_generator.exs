defmodule ScaleGenerator do
  ...
  def step([tonic, note | _], tonic, "m"), do: note
  def step([tonic, _, note | _], tonic, "M"), do: note
  def step([tonic, _, _, note | _], tonic, "A"), do: note
  def step([_ | tail], tonic, step), do: step(tail, tonic, step)
  ...
  def chromatic_scale(tonic \\ "C"), do:
    generate_scale(~w(C C# D D# E F F# G G# A A# B), tonic)

  defp generate_scale(scale, tonic) do
    tonic = String.capitalize(tonic)
    {first, second} = Enum.split_while(scale, &(&1 != tonic))
    second ++ first ++ [tonic]
  end
  ...
  def flat_chromatic_scale(tonic \\ "C"), do:
    generate_scale(~w(C Db D Eb E F Gb G Ab A Bb B), tonic)
  ...
  def scale(tonic, pattern) do
    ...
    [tonic | pattern
      |> String.codepoints
      |> Enum.scan(tonic, fn x, acc -> step(scale, acc, x) end)]
  end
end
